...
#pd.options.display.float_format = '{:,.2f}'.format #this method unlike next one, doesn't turn data values to strings
#goteborg = goteborg.map(lambda x: f"{x:.2f}")

#
#not essential to plot graph anymore as later done inside the dashboard callbacks
...
app.layout = html.Div([
    ...
    html.Hr(style={'height': '5px', 'background-color': 'white', 'border': 'none'}),
    html.H1(children = 'Please select either NO2 or PM10', style={'textAlign': 'center', 'color': 'black', 'fontSize': 15}),
...
def update_city_data(selectedcity, selectedyear):
    ...
    if not selectedcity or not selectedyear:
        raise PreventUpdate # -> unclear!! prevents the callback from firing if no dropdown selected
...
#________________________________________________________________________
#Callback for four cities' comparison graph at bottom
